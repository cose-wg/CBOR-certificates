...
/*  let public_key = PublicKey::from_slice(&[
  ...
  "l" => vec![loop_on_x509_cert(std::fs::read(second_arg).expect("No such file!"))],
  "ll" => loop_on_certs_from_tls(second_arg),
...
/******************************************************************************************************/
// make a TLS connection to get server certificate chain/bag
fn loop_on_certs_from_tls(domain_name: String) -> Vec<Cert> {
  let mut config = rustls::ClientConfig::new();
  config.root_store.add_server_trust_anchors(&webpki_roots::TLS_SERVER_ROOTS);
  let dns_name = webpki::DNSNameRef::try_from_ascii_str(&domain_name).unwrap();
  let mut sess = rustls::ClientSession::new(&std::sync::Arc::new(config), dns_name);
  ...
  tls.write_all(b"GET / HTTP/1.1").unwrap();
  tls.flush().unwrap();
  tls.sess.get_peer_certificates().unwrap().iter().map(|c| loop_on_x509_cert(c.0.clone())).collect()
}
/******************************************************************************************************/
/******************************************************************************************************/
// Parse a DER encoded X509 and encode it as C509, reverse it
fn loop_on_x509_cert(input: Vec<u8>) -> Cert {
  let oi = input.clone();
  let parsed_cert = parse_x509_cert(input);
  let reversed_cert = parse_c509_cert(lcbor_array(&parsed_cert.cbor), false);
  assert_eq!(oi, reversed_cert.der);
  println!("The input X.509 certificate was successfully encoded and reconstructed");
...
fn parse_x509_cert(input: Vec<u8>) -> Cert {
  ...
  if e == [0x43, 0x01, 0x00, 0x01] { //check for exponent == 65537
    output.push(n);
  } else {
...
fn cbor_time(b: &[u8]) -> Vec<u8> {
  let time_string = if b[0] == ASN1_UTC_TIME as u8 { [b"20", lder(b, ASN1_UTC_TIME)].concat() } else { lder(b, ASN1_GEN_TIME).to_vec() };
  let time_string = from_utf8(&time_string).unwrap();
  match time_string {
  ASN1_GEN_TIME_MAX => lcbor_simple(CBOR_NULL),
  _ => lcbor_uint(chrono::NaiveDateTime::parse_from_str(time_string, "%Y%m%d%H%M%SZ").unwrap().timestamp() as u64),
  }
}
...
fn cbor_general_names(b: &[u8], t: u8, opt: u8) -> Vec<u8> {
  ...
  println!("Storing context tag: {}", context_tag);
  ...
  for pqi in lder_vec(pi[1], ASN1_SEQ) {
    ...
    if let Some(pq_type) = pq_map(oid) {
        ...
        if pq_type == PQ_CPS {
            ...
            println!("THIS THIS IS THE TEXT {:02x?}", text);
...
fn cbor_ext_sct(b: &[u8], not_before: &[u8]) -> Vec<u8> {
  ...
  println!("ts, not_before_ms:\n{} {} ", ts, not_before_ms); //debug
...
//type Type = Vec<dyn std::any::Any>;
// Define the Name struct
struct Name<'a> {
  content_type: u8,
  text_content: &'a str,
  byte_content: &'a [u8],
  array_content: &'a Vec<Value>,
}
fn parse_c509_cert(input: Vec<u8>, is_str: bool) -> Cert {
  println!("INPUT: {:x?} with len {}", input, input.len());
  let bytes = {
  if is_str== true {
  match hex::decode(input) {
  Ok(b) => {
  println!("Decoded bytes: {:x?}", b);
  b
  }
  Err(err) => {
  ...
  let mut x509_certificate: Vec<u8> = Vec::new();
  let mut certificate_vec: Vec<Vec<u8>> = Vec::new();
  let mut tbs_cert_vec: Vec<Vec<u8>> = Vec::new();
  let mut dummy: Vec<Vec<u8>> = Vec::new();
    
  let cursor = Cursor::new(bytes);
  match serde_cbor::de::from_reader(cursor) {
  Ok(value) => {
  if let Value::Array(elements) = value {
  println!("CBOR array contains {} elements", elements.len());
  for i in 0..11 {
   println!("Will try to parse &elements[i]:  {:?}", &elements[i]);
  }
  ...
  println!("Element: {:?}", elements[0]);
  match elements[0] {
  Value::Integer(version) => {
  println!("Type with CBOR integer value: {}", version); // {}", i);
  if version != C509_TYPE_X509_ENCODED as i128 {
  panic!("The version value can only handle certs of type {}.");
  }
  ...
  println!("Done parsing sig_alg & sig_val: {:02x?}", sig_val);
  ...
  println!("Done parsing subject: {:02x?}", subject);
 ...
 println!("Done parsing extensions: {:02x?}", extensions);
 
 tbs_cert_vec.push(extensions);
 
//for (i, element) in tbs_cert_vec.into_iter().enumerate() {  println!("\nPosition {} = \n{:02x?}", i, element); } std::process::exit(0); //Debug
 ...
 //... and the actual reconstructed signature
  //println!("\nsig_val: {:02x?}", sig_val); std::process::exit(0); //Debug
 certificate_vec.push(sig_val);
 //println!("Done parsing tbs_cert_vec: {:02x?}", certificate_vec.get(0));
...
*/
  ...
  println!("The value is not a CBOR array.");
  }
  }
  Err(err) => {
  ...
  println!("Done reconstructing X.509: {:02x?}", x509_certificate);
//std::process::exit(0);
  // println!("The CBOR data represents an array: {}.", value);
  //https://users.rust-lang.org/t/comparison-of-way-too-many-rust-asn-1-der-libraries/58683/3
  //https://docs.rs/x509-certificate/latest/x509_certificate/certificate/struct.X509CertificateBuilder.html
...
fn parse_cbor_eui64(cn: &[u8]) -> Vec<u8> {
  ...
  println!("CBOR eui64: {:x?}", my_string);
  my_string.into_bytes()
}
//***************************************************************************************************************************************
//***************************************************************************************************************************************
fn parse_cbor_name<'a>(input: &'a Value, empty_vec: &'a Vec<Value>) -> Vec<u8> {
  println!("Incoming value: {:x?}", input);
  let mut result_vec = Vec::new();
    
  match input {
  Value::Text(name) => {
  println!("CBOR name: {:x?}", name);
  let cn = name.as_bytes();
    
  let attr_type_and_val =
  lder_to_two_seq(ATT_COMMON_NAME_OID.to_der_vec().unwrap(), lder_to_generic(cn.to_vec(), ASN1_UTF8_STR));
  result_vec.push(lder_to_generic(attr_type_and_val, ASN1_SET));
  },
  Value::Bytes(b) => {
  println!("CBOR bytes: {:x?}", b);
  let cn = match b[0] {
...
fn parse_cbor_time(input: &Value) -> (Vec<u8>, i64) {
  ...
  } else { //using two digit year format to match UTC time format
  (ts.format("%y%m%d%H%M%SZ").to_string(), *val)
   }
  }
  Value::Null => {
...
/*    
   let dt = chrono::TimeZone::timestamp(&chrono::Utc, match input {
  Value::Integer(val) => {
  println!("CBOR int DAFUK: {:x?}", val);
  *val as i64
  }
  Value::Null => {
  println!("CBOR NULL");
  ...
  */  
  println!("Converted POSIX timestamp to custom format: {}", formatted_date);
...
pub fn parse_cbor_pub_key(pub_key: &Value, key_type: i64) -> Vec<u8> {
 ...
 let mut pub_key_vec = Vec::new();
  ...
  _ => println!("pub key in non-byte format"),
  }
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_RSA_ENC_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_NULL.to_vec()
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_SECP256R_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_SECP256R_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_SECP384R_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_SECP384R_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_SECP521R_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_SECP521R_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_generic(
  lder_to_generic(PK_X25519_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_generic(
  lder_to_generic(PK_X448_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_generic(
  lder_to_generic(PK_ED25519_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_generic(
  lder_to_generic(PK_ED448_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_generic(   
  lder_to_generic(PK_HSS_LMS_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_generic(   
  lder_to_generic(PK_XMSS_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_generic(   
  lder_to_generic(PK_XMSS_MT_OID.as_bytes().to_vec(), ASN1_OID),
  ASN1_SEQ
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_BRAINPOOL256R1_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_BRAINPOOL256R1_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_BRAINPOOL384R1_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_BRAINPOOL384R1_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_BRAINPOOL512R1_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_BRAINPOOL512R1_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_FRP256V1_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_FRP256V1_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  ...
  result.push(
  lder_to_two_seq(   
  lder_to_generic(PK_SM2P256V1_OID.as_bytes().to_vec(), ASN1_OID),
  lder_to_generic(PK_SM2P256V1_PARAM_OID.as_bytes().to_vec(), ASN1_OID)
  )
  );
  check_and_reconstruct_pub_key_ecc(pub_key_vec, PK_SM2P256V1)
   
  },
  _ => { panic!("Could not parse public key"); result.push([0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01, 0x05, 0x00].to_vec()); }
  }
 } ; //end of let dummy
 result.push(dummy);
 //println!("SPK:\n{:02x?}", lder_to_seq(result)); std::process::exit(0);
 lder_to_seq(result)
}
//***************************************************************************************************************************************
//***************************************************************************************************************************************
fn check_and_reconstruct_pub_key_ecc(mut pub_key: Vec<u8>, ecc_type_id: i64) -> Vec<u8> {
...
//  if (y[y.len() - 1] & 1 == 0 && is_even == false) || y[y.len() - 1] & 1 == 1 && is_even == true {
  ...
  //let y_inv = &mc.p-&y;
  let (_, yb) = y.to_bytes_be();
 
  //let (_, ysb) = ys.to_bytes_be();  let (_, yib) = y_inv.to_bytes_be();
  //let yb_inv= y.to_bytes_be();
  println!("yb:\n{:02x?}", yb);
...
fn check_and_reconstruct_pub_key_rsa(pub_key: &Value, key_id: i64) -> Vec<u8> {
  let mut modulus;
  let mut exponent;
  ...
  println!("Parsed RSA mdule as {:02x?}", pub_key_mod_only);
//  let my_integer = Integer::from_bytes_be(pub_key_mod_only);
  ...
  _ => { panic!("Could not decode rsa pub key: {:?}", pub_key); }
  }
  //println!("modulus\n{:02x?}", modulus);  println!("exponent\n{:02x?}", exponent);  
  //println!("inner sec\n{:02x?}", lder_to_two_seq(modulus, exponent));
 
  //println!("as bitstr\n{:02x?}", lder_to_bit_str(lder_to_two_seq(modulus, exponent)));
  //std::process::exit(0);
  //lder_to_generic(lder_to_two_seq(modulus, exponent), ASN1_BIT_STR)
  lder_to_bit_str(lder_to_two_seq(modulus, exponent))
  //panic!("Reconstruction of rsa pub keys not yet supported: {}", key_id);
}
fn check_and_reconstruct_pub_key_mac(mut pub_key: Vec<u8>, key_id: i64) -> Vec<u8> {
...
fn parse_cbor_extensions(input: &Value, ts_offset: i64) -> Vec<u8> {
  ...
  println!("Found ext of bytetype {:?}", raw_ext_type_oid);
  raw_ext_type_oid.to_vec()
  }
...
pub fn parse_cbor_sig_info(sig_alg: &Value, sig_val: &Value) -> (Vec<u8>, Vec<u8>){
  ...
  Value::Bytes(sig_val_bytes) => {
  sig_val_bytes.to_vec()
  },
  _ => panic!("Could not parse sig val"),
  };
  let mut parsed_sig_val = Vec::new();
  let mut param  = Vec::new();
 
  println!("sig val vec bytes: {:x?}", sig_val_vec);
  match sig_alg {
  Value::Integer(sign_alg_id) => {
  println!("Working with sign alg_id {}", sign_alg_id);
  match (*sign_alg_id as i64){
    SIG_RSA_V15_SHA1 => {
      oid = SIG_RSA_V15_SHA1_OID.as_bytes().to_vec();
    ...
    SIG_ECDSA_SHA256 => {
      ...
      parsed_sig_val = parse_cbor_ecc_sig_value(sig_val_vec.clone());
      println!("Encoded sig val bytes: {:02x?}", parsed_sig_val);
       //  panic!("not now");
    },
  SIG_ECDSA_SHA384 => {
...
//Some odd ones, not yet supported
  SIG_HSS_LMS => {
      ...
      //parsed_sig_val = parse_cbor_mac_sig_value(sig_val_vec.clone());      println!("Encoded sig val bytes: {:02x?}", parsed_sig_val);
...
pub fn parse_cbor_ecc_sig_value(sig_val_bytes: Vec<u8>) -> Vec<u8>{
  ...
  let midpoint = sig_val_bytes.len()/2;
//  let b: Vec<u8> = sig_val_bytes[0..32].to_vec();
//  let b: u32 = u32::from_le_bytes(sig_val_bytes[0..32]);
//let bytes = sig_val_bytes[0..32];//[ 0x02, 0x03, 0x01, 0x00, 0x01,  0x02, 0x03, 0x01, 0x00, 0x00,];
//let (rem, obj1) = Integer::from_ber(&sig_val_bytes[0..32]).expect("parsing failed");
  let mut r = sig_val_bytes[0..midpoint].to_vec();
  let mut s = sig_val_bytes[midpoint..sig_val_bytes.len()].to_vec();
  //r.insert(0, 0x00); //TODO
//  r.insert(0, 0x21);
//  r.insert(0, 0x02);
  //s.insert(0, 0x00); //TODO
//  s.insert(0, 0x21);
//  s.insert(0, 0x02);
  // println!("First half : {:02x?}", r);
  // println!("Second half: {:02x?}", s);
//let (rem, obj2) = Integer::from_ber(&r).expect("parsing failed");
//  result.push(lder_to_int(r));  result.push(lder_to_int(s));
  result.push(lder_to_pos_int(r));  result.push(lder_to_pos_int(s));
 
//assert_eq!(obj1, Integer::from_u32(65537));
  //let test = Integer::from_u32(b);
  // bytes(sig_val_bytes.get(0..midpoint));
 
  //let my_bitstring = BitString::from_bytes(&[0b11001100, 0b10101010], 4);
  // Encode the ASN.1 BIT STRING
  //let encoded_bytes: Vec<u8> = obj1.to_der().unwrap();
  //obj1.write_der(&mut writer);
...
pub fn parse_cbor_mac_sig_value(sig_val_bytes: Vec<u8>) -> Vec<u8>{
...
pub fn cleanup(mut file_contents: Vec<u8>) -> Vec<u8> {
  ...
  let cleaned_contents = if let Some(last_byte) = file_contents.last() {
  if *last_byte == b'\n' {
...
fn parse_cbor_ext_subject_key_id(extension_val: &Value, critical: bool) -> Vec<u8> {
  ...
  let mut ext_val_arr = match extension_val {
  Value::Bytes(raw_val) => {
   lder_to_generic(raw_val.to_vec(), ASN1_OCTET_STR)
  }
...
fn parse_cbor_general_name(extension_val: &Value) -> Vec<u8> {
  ...
  match general_name[i] {
    ...
    1 => {  //rfc822Name == text
      println!("1, noice");
      match &general_name[i+1] {
      Value::Text(rfc_822_name) => {
        lder_to_generic(rfc_822_name.as_bytes().to_vec(), ASN1_INDEX_ONE_EXT)
      },
      _ => panic!("Error parsing value: {:?}.", general_name[i+1]),
     }
    },
    2 => {  //dnsName == text
       ...
       match &general_name[i+1] {
        Value::Text(dns_name) => {
          lder_to_generic(dns_name.as_bytes().to_vec(), ASN1_INDEX_TWO_EXT)
        },
        _ => panic!("Error parsing value: {:?}.", general_name[i+1]),
        }
    },
    4 => { // directoryName == Name
      println!("4, noice");
      parse_cbor_name(&general_name[i+1] as &Value, &empty_vec)
      //todo test more
    },
    6 => { // uri == text
       println!("6, noice");
       match &general_name[i+1] {
        Value::Text(uri) => {
          lder_to_generic(uri.as_bytes().to_vec(), ASN1_URL)
        },
        _ => panic!("Error parsing value: {:?}.", general_name[i+1]),
        }
    },
    7 => { //ipAddress == bytes
    println!("7, noice"); Vec::new()
    },
    8 => { //registeredID  == ~oid
      println!("8, noice"); Vec::new()
    },
    _ => { panic!("Not implemented: {:?}.", general_name[i]) },
  }
  },
  ...
  let general_name = lder_to_seq(general_name_arr);
  lder_to_generic(general_name, ASN1_OCTET_STR)
  }
/*
...
*/
  Value::Text(raw_val) => {
  lder_to_generic(lder_to_generic(lder_to_generic(raw_val.as_bytes().to_vec(), ASN1_INDEX_TWO_EXT), ASN1_SEQ), ASN1_OCTET_STR)
  }
  _ => panic!("Error parsing value: {:?}.", extension_val),
  }
}
...
fn parse_cbor_ext_basic_constraints(extension_val: &Value, critical: bool) -> Vec<u8> {
  ...
  //panic!("work is here: {:02x?}", lder_to_two_seq(oid, ASN1_X509_BASIC_CONSTRAINT_TRUE.to_der_vec().unwrap()));
    //lder_to_two_seq(oid, ASN1_X509_BASIC_CONSTRAINT_TRUE.to_vec())
    //WAS: lder_to_two_seq(oid, ASN1_X509_BASIC_CONSTRAINT_TRUE.to_der_vec().unwrap())
    lder_to_generic(ASN1_X509_CRITICAL.to_vec(), ASN1_SEQ)
  } else {
  ...
  //panic!("found path len: {}", path_len);
  }
  },
  _ => panic!("Illegal path len {:?}", extension_val)
  }    
  //lder_to_two_seq(oid, ASN1_BASIC_CONSTRAINT_FALSE.to_vec()) //TODO
 
  }; //end let second
  lder_to_two_seq(oid, second)
  } else {
   lder_to_two_seq(oid, ASN1_X509_BASIC_CONSTRAINT_FALSE.to_vec())
  }
...
fn parse_cbor_ext_cert_policies(extension_val: &Value, critical: bool) -> Vec<u8> {
  ...
  println!("FOUND pol id: {:02x?}", map_cert_policy_id_to_oid(*pol_id as i64));
  ...
  let q_oid = {
  match specifiers.get(0).unwrap() {
    Value::Bytes(raw_oid) => {
      lder_to_generic(raw_oid.to_vec(), ASN1_OID)
    },
    Value::Integer(pol_id) => {
      if PQ_CPS == *pol_id as i64 {
        println!("FOUND THIS cps {:02x?}", PQ_CPS_OID.to_der_vec().unwrap());
        text_type = ASN1_IA5_SRT;
        PQ_CPS_OID.to_der_vec().unwrap()
      } else if PQ_UNOTICE == *pol_id  as i64{
        println!("FOUND THIS unotice {:02x?}", PQ_UNOTICE_OID.to_der_vec().unwrap());
  ...
  let q_text = {
  match specifiers.get(1).unwrap() {
    Value::Text(qualifier) => {
      println!("FOUND THIS text {:02x?}", qualifier.as_bytes());
      lder_to_generic(qualifier.as_bytes().to_vec(),text_type)
    },
    _ => {
...
fn map_cert_policy_id_to_oid(cert_policy_id: i64) -> Vec<u8> {
  ...
  CP_ANY_POLICY => {
  CP_ANY_POLICY_OID.to_der_vec().unwrap()
  },
  CP_DOMAIN_VALIDATION => {
  CP_DOMAIN_VALIDATION_OID.to_der_vec().unwrap()
  },
  CP_ORGANIZATION_VALIDATION => {
  CP_ORGANIZATION_VALIDATION_OID.to_der_vec().unwrap()
  },
  CP_INDIVIDUAL_VALIDATION => {
  CP_INDIVIDUAL_VALIDATION_OID.to_der_vec().unwrap()
  },
  CP_EXTENDED_VALIDATION => {
  CP_EXTENDED_VALIDATION_OID.to_der_vec().unwrap()
  },
  CP_RESOURCE_PKI => {
  CP_RESOURCE_PKI_OID.to_der_vec().unwrap()
  },
  CP_RESOURCE_PKI_ALT => {
  CP_RESOURCE_PKI_ALT_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_CI => {
  CP_RSP_ROLE_CI_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_EUICC => {
  CP_RSP_ROLE_EUICC_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_EUM => {
  CP_RSP_ROLE_EUM_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_DP_TLS => {
  CP_RSP_ROLE_DP_TLS_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_DP_AUTH => {
  CP_RSP_ROLE_DP_AUTH_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_DP_PB => {
  CP_RSP_ROLE_DP_PB_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_DS_TLS => {
  CP_RSP_ROLE_DS_TLS_OID.to_der_vec().unwrap()
  },
  CP_RSP_ROLE_DS_AUTH => {
  CP_RSP_ROLE_DS_AUTH_OID.to_der_vec().unwrap()
  },
...
fn parse_cbor_ext_auth_key_id(extension_val: &Value, critical: bool) -> Vec<u8> {
  ...
  Value::Bytes(raw_val) => {
   lder_to_generic(
  lder_to_generic(raw_val.to_vec(), ASN1_INDEX_ZERO_EXT),
   ASN1_SEQ
  )
  },
  Value::Array(array) => {
  panic!("Can't handle arrays yet: {:?}.", extension_val)
  }
  _ => panic!("Error parsing value: {:?}.", extension_val),
  };
  lder_to_two_seq(oid, lder_to_generic(ext_val_arr, ASN1_OCTET_STR))
  ...
  Value::Array(elements) => {
    for element in elements {
      match element {
        ...
        Value::Bytes(raw_val) => {
          ext_val_arr.push(lder_to_generic(raw_val.to_vec(),ASN1_OID)); //todo, check handling of multiple OIDs
        },
...
fn map_key_purpose_id_to_oid(key_purpose_id: u64) -> Vec<u8> {
  ...
  let dummy = EKU_TLS_SERVER_OID.to_der_vec().unwrap();
  match key_purpose_id {
  EKU_TLS_SERVER => {
  EKU_TLS_SERVER_OID.to_der_vec().unwrap()
  },
  EKU_TLS_CLIENT => {
  EKU_TLS_CLIENT_OID.to_der_vec().unwrap()
  },
  EKU_CODE_SIGNING => {
  EKU_CODE_SIGNING_OID.to_der_vec().unwrap()
  },
  EKU_EMAIL_PROTECTION => {
  EKU_EMAIL_PROTECTION_OID.to_der_vec().unwrap()
  },
  EKU_TIME_STAMPING => {
  EKU_TIME_STAMPING_OID.to_der_vec().unwrap()
  },
  EKU_OCSP_SIGNING => {
  EKU_OCSP_SIGNING_OID.to_der_vec().unwrap()
  },
  EKU_ANY_EKU => {
  EKU_ANY_EKU_OID.to_der_vec().unwrap()
  },
  EKU_KERBEROS_PKINIT_CLIENT_AUTH => {
  EKU_KERBEROS_PKINIT_CLIENT_AUTH_OID.to_der_vec().unwrap()
  },
  EKU_KERBEROS_PKINIT_KDC => {
  EKU_KERBEROS_PKINIT_KDC_OID.to_der_vec().unwrap()
  },
  EKU_SSH_CLIENT => {
  EKU_SSH_CLIENT_OID.to_der_vec().unwrap()
  },
  EKU_SSH_SERVER => {
  EKU_SSH_SERVER_OID.to_der_vec().unwrap()
  },
  EKU_BUNDLE_SECURITY => {
  EKU_BUNDLE_SECURITY_OID.to_der_vec().unwrap()
  },
  EKU_CMC_CERT_AUTHORITY => {
  EKU_CMC_CERT_AUTHORITY_OID.to_der_vec().unwrap()
  },
  EKU_CMC_REG_AUTHORITY => {
  EKU_CMC_REG_AUTHORITY_OID.to_der_vec().unwrap()
  },
  EKU_CMC_ARCHIVE_SERVER => {
  EKU_CMC_ARCHIVE_SERVER_OID.to_der_vec().unwrap()
  },
  EKU_CMC_KEY_GEN_AUTHORITY => {
  EKU_CMC_KEY_GEN_AUTHORITY_OID.to_der_vec().unwrap()
  },
...
fn parse_cbor_ext_auth_info(extension_val: &Value, critical: bool) -> Vec<u8> {
  ...
  Value::Array(elements) => {
  let mut wip = Vec::new();
  println!("{} and {}", elements.len(), elements.len() % 2);
  ...
  println!("FOUND access_method: {:02x?}", map_auth_info_id_to_oid(*access_method as i64));
...
fn map_auth_info_id_to_oid(access_method: i64) -> Vec<u8> {
  ...
  INFO_OCSP => {
  INFO_OCSP_OID.to_der_vec().unwrap()
  },
  INFO_CA_ISSUERS => {
  INFO_CA_ISSUERS_OID.to_der_vec().unwrap()
  },
  INFO_TIME_STAMPING => {
  INFO_TIME_STAMPING_OID.to_der_vec().unwrap()
  },
  INFO_CA_REPOSITORY => {
  INFO_CA_REPOSITORY_OID.to_der_vec().unwrap()
  },
  INFO_RPKI_MANIFEST => {
  INFO_RPKI_MANIFEST_OID.to_der_vec().unwrap()
  },
  INFO_SIGNED_OBJECT => {
  INFO_SIGNED_OBJECT_OID.to_der_vec().unwrap()
  },
  INFO_RPKI_NOTIFY => {
  INFO_RPKI_NOTIFY_OID.to_der_vec().unwrap()
  },
...
let mut total_tally = 0;
 let ts_os_ms = 1000*ts_offset as i64;
  ...
  _ => { panic!("Can't parse {:?}", sct_array.get(i+2)); }
  }
  match sct_array.get(i+3).unwrap() {
   Value::Bytes(sig_val) => {
  println!("Found sigVal bytes {:02x?}", sig_val);
  let r = lder_to_pos_int(sig_val[0..sig_val.len()/2].to_vec());
  let s_r = sig_val[sig_val.len()/2..sig_val.len()].to_vec();
  let s = lder_to_pos_int(s_r.clone());
  println!("s_r {:02x?}", s_r);
  let seq = lder_to_two_seq(r, s);
  println!("After seq {:02x?}", seq);
  ext_val_arr.push(0x00);
  ...
  _ => { panic!("Can't parse {:?}", sct_array.get(i+3)); }
  }
 }
  ext_val_arr.insert(total_tally, 0x00); //the size field for the last 4-touple
  ext_val_arr.insert(total_tally+1,sct_size as u8);
 
  println!("After all loop {:02x?}", ext_val_arr);
   },
  _ => panic!("Error parsing value: {:?}.", extension_val),
  };
  ext_val_arr = lder_to_generic(
  lder_to_generic(sct_add_len(ext_val_arr), ASN1_OCTET_STR),
  ASN1_OCTET_STR
  );
  println!("After os {:02x?}", ext_val_arr);
 
  //panic!("EXT_SCT_LIST construction is wip, not currently correct!");
  //print_str_warning("EXT_SCT_LIST construction is wip, not currently correct!");
  lder_to_two_seq(oid, ext_val_arr)
}
//***************************************************************************************************************************************
//***************************************************************************************************************************************
...
//  let ext_val_arr = match hw_module {
  match hw_module {
  Value::Array(array) => {
  if let Value::Bytes(raw_oid) = &array[0] {
  println!("Working with raw_oid.to_vec() {:02x?}", raw_oid.to_vec());
  inner_vec.push(lder_to_generic(raw_oid.to_vec(), ASN1_OID));
  if let Value::Bytes(raw_val) = &array[1] {
  println!("Working with raw_val.to_vec() {:02x?}", raw_val.to_vec());
...
pub mod lder {
  ...
  let mut vec = Vec::new();
  println!("lder_vec is parsing {:02X?}", b);
...
//  result.extend(item);
//  }
//  result.insert(0, result.len() as u8); //TODO handle >255
//   result.insert(0, ASN1_SEQ);
//  result
  }
  ...
  result.insert(0, result.len() as u8); //TODO handle >two byte len?
   
  if 127 < result.len() && result.len() < 255 {
  result.insert(0, ASN1_ONE_BYTE_SIZE);    
  } else if 255 < result.len() {
